import { ColorType, Font, getDefaultFont, getFallbackFontName, mm2pt, PDFRenderProps } from "@pdfme/common";
import { PDFDocument, PDFFont } from "@pdfme/pdf-lib";
import { convertForPdfLayoutProps, hex2PrintingColor, rotatePoint } from "../utils.js";
import { DEFAULT_ALIGNMENT, DEFAULT_CHARACTER_SPACING, DEFAULT_FONT_COLOR, DEFAULT_FONT_SIZE, DEFAULT_LINE_HEIGHT, DEFAULT_VERTICAL_ALIGNMENT, VERTICAL_ALIGN_BOTTOM, VERTICAL_ALIGN_MIDDLE, VERTICAL_ALIGN_TOP } from "./constants.js";
import { calculateDynamicFontSize, getFontDescentInPt, getFontKitFont, heightOfFontAtSize, splitTextToSize, widthOfTextAtSize } from "./helper.js";
import type { TextSchema } from "./types";

const embedAndGetFontObj = async (arg: { pdfDoc: PDFDocument; font: Font; _cache: Map<any, any> }) => {
  ...
  const fontObj = Object.keys(font).reduce((acc, cur, i) => Object.assign(acc, { [cur]: fontValues[i] }), {} as { [key: string]: PDFFont });

  _cache.set(pdfDoc, fontObj);
  return fontObj;
};

const getFontProp = async ({ value, font, schema, colorType, _cache }: { value: string; font: Font; colorType?: ColorType; schema: TextSchema; _cache: Map<any, any> }) => {
  const fontSize = schema.dynamicFontSize ? await calculateDynamicFontSize({ textSchema: schema, font, value, _cache }) : (schema.fontSize ?? DEFAULT_FONT_SIZE);
...
export const pdfRender = async (arg: PDFRenderProps<TextSchema>) => {
  ...
  const [pdfFontObj, fontKitFont, fontProp] = await Promise.all([embedAndGetFontObj({ pdfDoc, font, _cache }), getFontKitFont(schema.fontName, font, _cache), getFontProp({ value, font, schema, _cache, colorType })]);
